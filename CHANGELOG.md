...
- Suppress delayed completion notifications after `results` has already returned the completed session.
...
- Add projection coverage for `defaultResumable`, normalized inventory statuses, and resolved `effectiveConfig` values.
- Add lifecycle coverage for removal visibility and stale notifications after removal or result retrieval.
