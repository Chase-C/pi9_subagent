...
function statusIcon(run?: RunSnapshot): string { if (!run) return "○"; if (run.status.kind === "queued") return "◷"; if (run.status.kind === "running") return "●"; return run.status.outcome === "completed" ? "✓" : "✗"; }
