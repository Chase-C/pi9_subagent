...
- Add strict input validation requiring at least one selectable option, structured answer details, model guidance that routes open-ended questions to normal responses, and graceful handling when interactive UI is unavailable.
...
- Add integer-millisecond timeouts from `timeout` or `PI9_ASK_TIMEOUT_MS`, with explicit precedence, zero disabling, structured `unanswered` results, and one deadline across the full interaction.
